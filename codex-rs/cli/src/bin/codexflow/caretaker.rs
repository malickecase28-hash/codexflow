...
enum CaretakerCommand {
    ...
    Queue { #[arg(long)] apply: bool },
    WorkflowInstall { #[arg(long)] force: bool },
}
...
pub fn handle(project_root: &Path, args: CaretakerArgs) -> Result<()> {
    match args.command {
        ...
        CaretakerCommand::Show => {
            println!("{}", serde_json::to_string_pretty(&load_policy(project_root)?)?);
            Ok(())
        }
        CaretakerCommand::Scan { json } => {
            let report = scan(project_root)?;
            if json { println!("{}", serde_json::to_string_pretty(&report)?); } else { print_human(&report); }
            Ok(())
        }
...
fn init(project_root: &Path, force: bool) -> Result<()> {
    ...
    if path.exists() && !force { bail!("caretaker policy already exists at {}; use --force to replace it", path.display()); }
    fs::create_dir_all(path.parent().context("caretaker policy parent")?)?;
    fs::write(&path, serde_json::to_vec_pretty(&CaretakerPolicy::default())?).with_context(|| format!("write {}", path.display()))?;
...
fn scan(project_root: &Path) -> Result<ScanReport> {
    ...
    let files = tracked.split('\0').filter(|value| !value.is_empty()).map(PathBuf::from).collect::<Vec<_>>();
    ...
    for relative in &files {
        ...
        if is_source_path(relative) {
            if let Ok(metadata) = fs::metadata(&path) && metadata.len() > policy.max_source_file_bytes {
            ...
            if let Ok(text) = fs::read_to_string(&path) {
                let count = text.lines().filter(|line| line.contains("TODO") || line.contains("FIXME")).count();
                if count >= policy.todo_threshold { todo_hits.insert(relative.display().to_string(), count); }
            }
        }
        if let Some(name) = relative.file_name() { basenames.entry(name.to_string_lossy().to_ascii_lowercase()).or_default().push(relative.display().to_string()); }
    }
    ...
    if has_rust_project(project_root) && !project_root.join(".codexflow").join("build.json").exists() {
    ...
    if !project_root.join(".codexflow").join("orchestration.json").exists() && !project_root.join("docs").join("maintenance").join("departments.json").exists() {
    ...
    findings.sort_by(|left, right| risk_rank(&right.risk).cmp(&risk_rank(&left.risk)).then_with(|| left.kind.cmp(&right.kind)).then_with(|| left.id.cmp(&right.id)));
    Ok(ScanReport { schema: "codexflow.caretaker.scan.v1", project_root: project_root.display().to_string(), clean_worktree, tracked_files: files.len(), findings })
}

fn queue(project_root: &Path, apply: bool) -> Result<()> {
    let policy = load_policy(project_root)?;
    let report = scan(project_root)?;
    if !report.clean_worktree { bail!("caretaker queue is blocked by a dirty worktree"); }
    let mut queued = Vec::new();
    for finding in report.findings {
        if !policy.allowed_queue_risks.contains(&finding.risk) { continue; }
        let task_id = format!("maint_{}", finding.id);
        if apply {
            let gates = if finding.risk == "medium" { vec!["independent_review".to_string()] } else { Vec::new() };
            runtime_state::seed_orchestration_plan(project_root, &task_id, &finding.summary, &finding.risk, &gates)?;
        }
...
fn install_workflow(project_root: &Path, force: bool) -> Result<()> {
    let path = project_root.join(".github").join("workflows").join("codexflow-caretaker.yml");
    if path.exists() && !force { bail!("workflow already exists at {}; use --force to replace it", path.display()); }
...
fn policy_path(project_root: &Path) -> PathBuf { project_root.join(".codexflow").join("caretaker.json") }
fn load_policy(project_root: &Path) -> Result<CaretakerPolicy> { let path = policy_path(project_root); if !path.exists() { return Ok(CaretakerPolicy::default()); } let data = fs::read_to_string(&path).with_context(|| format!("read {}", path.display()))?; serde_json::from_str(&data).with_context(|| format!("parse {}", path.display())) }

fn finding(kind: &str, risk: &str, summary: &str, paths: Vec<String>, recommendation: &str) -> Finding {
    let mut hasher = DefaultHasher::new(); kind.hash(&mut hasher); summary.hash(&mut hasher); paths.hash(&mut hasher);
    Finding { id: format!("{:08x}", hasher.finish() as u32), kind: kind.to_string(), risk: risk.to_string(), summary: summary.to_string(), paths, recommendation: recommendation.to_string() }
}

fn print_human(report: &ScanReport) { println!("project: {}", report.project_root); println!("tracked files: {}", report.tracked_files); println!("clean worktree: {}", report.clean_worktree); if report.findings.is_empty() { println!("findings: none"); return; } println!("findings:"); for finding in &report.findings { println!("  [{}] {} {} - {}", finding.risk, finding.id, finding.kind, finding.summary); for path in &finding.paths { println!("      {path}"); } } }
...
fn is_source_path(path: &Path) -> bool { matches!(path.extension().and_then(|value| value.to_str()), Some("rs" | "py" | "ts" | "tsx" | "js" | "jsx" | "go" | "java" | "kt" | "c" | "cc" | "cpp" | "h" | "hpp" | "cs" | "swift")) }
fn is_common_basename(name: &str) -> bool { matches!(name, "mod.rs" | "lib.rs" | "main.rs" | "index.ts" | "index.tsx" | "index.js" | "readme.md" | "license" | "package.json" | "cargo.toml") }
fn has_rust_project(project_root: &Path) -> bool { if project_root.join("Cargo.toml").is_file() { return true; } fs::read_dir(project_root).ok().into_iter().flatten().filter_map(Result::ok).any(|entry| entry.path().is_dir() && entry.path().join("Cargo.toml").is_file()) }
fn risk_rank(risk: &str) -> u8 { match risk { "critical" => 4, "high" => 3, "medium" => 2, "low" => 1, _ => 0 } }
