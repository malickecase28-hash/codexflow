...
pub fn handle(project_root: &Path, args: DeliveryArgs) -> Result<()> {
    match args.command {
        ...
        DeliveryCommand::WorktreeCreate {
            task,
            base,
            branch,
            path,
        ...
        DeliveryCommand::WorktreeRemove {
            task,
            path,
            delete_branch,
            yes,
        } => worktree_remove(project_root, &task, path.as_deref(), delete_branch, yes),
        DeliveryCommand::WorktreeList => run_git(project_root, &["worktree", "list"]),
        DeliveryCommand::PrCreate {
            title,
            body,
            body_file,
            base,
...
fn worktree_create(
    ...
    let branch = branch
        .map(str::to_string)
        .unwrap_or_else(|| format!("codexflow/task/{task}"));
    let path = explicit_path
        .map(Path::to_path_buf)
        .unwrap_or_else(|| default_worktree_path(project_root, task));
    if path.exists() {
    ...
    println!(
        "{}",
        serde_json::json!({"task": task, "branch": branch, "path": path, "base": base_ref})
    );
    Ok(())
}
...
fn worktree_remove(
    ...
    if !yes {
        bail!("worktree removal requires --yes");
    }
    validate_task_id(task)?;
    let path = explicit_path
        .map(Path::to_path_buf)
    ...
    if delete_branch {
        let status = Command::new("git")
            ...
            .status()
            .context("delete task branch")?;
        if !status.success() {
            bail!("worktree removed but branch deletion failed with {status}");
        }
    }
    Ok(())
}
...
fn pr_checks(project_root: &Path, watch: bool) -> Result<()> {
    ...
    if !status.success() {
        bail!("required pull-request checks are not passing");
    }
    Ok(())
}
...
fn merge_check(project_root: &Path, allow_unverified: bool) -> Result<()> {
    ...
    let status = Command::new("gh")
        ...
        .args(["pr", "checks", "--required"])
        .status()
        .context("check required pull-request checks")?;
    if !status.success() {
        bail!("merge blocked: required pull-request checks are not passing");
    }
    let output = Command::new("gh")
        ...
        .args([
            ...
            "view",
            "--json",
            "number,state,isDraft,mergeStateStatus,reviewDecision,url",
        ])
        .output()
        .context("inspect pull request")?;
    if !output.status.success() {
        bail!("cannot inspect current pull request");
    }
...
fn ensure_clean_commit_state(project_root: &Path) -> Result<()> {
    ...
    if !status.trim().is_empty() {
        bail!("working tree is dirty; commit or stash before PR/merge operations");
    }
    let branch = git_output(project_root, &["branch", "--show-current"])?;
    if branch.trim().is_empty() {
        bail!("detached HEAD cannot be delivered as a normal pull request");
    }
    Ok(())
}
...
fn ensure_git_repo(project_root: &Path) -> Result<()> {
    ...
    if !status.success() {
        bail!("project root is not inside a git worktree");
    }
    Ok(())
}
