...
pub fn handle(project_root: &Path, args: OrchestrateArgs) -> Result<()> {
    match args.command {
        ...
        OrchestrateCommand::Lease(args) => lease::handle(project_root, args),
    }
}
...
fn plan(
    ...
    let lease_scope = task_id.as_deref().map(lease::task_scope).transpose()?;
...
mod tests {
    ...
    #[test]
    fn plan_emits_validated_task_lease_scope() {
