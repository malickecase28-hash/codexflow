...
fn context_hygiene_enabled() -> bool {
    env_flag("CODEX_CONTEXT_HYGIENE")
}

fn drop_stale_skills_enabled() -> bool {
    std::env::var("CODEX_HYGIENE_DROP_STALE_SKILLS")
        .ok()
        .map(|_| env_flag("CODEX_HYGIENE_DROP_STALE_SKILLS"))
        .unwrap_or(true)
}

fn is_selected_skill_context_item(item: &ResponseItem) -> bool {
...
fn is_skill_catalog_context_item(item: &ResponseItem) -> bool {
    let ResponseItem::Message { role, content, .. } = item else {
        return false;
    };
    role == "developer"
...
impl ContextManager {
    ...
    /// Build the model-visible prompt snapshot with optional context hygiene.
    ///
    /// This mutates only the cloned ContextManager consumed by the request. The
    /// session's durable history and rollout remain unchanged.
    pub(crate) fn for_prompt_hygienic(
        mut self,
        input_modalities: &[InputModality],
        current_turn_id: &str,
    ) -> Vec<ResponseItem> {
        self.normalize_history(input_modalities);
        if context_hygiene_enabled() {
            if drop_stale_skills_enabled() {
                self.prune_stale_skill_context(current_turn_id);
            }
            self.prune_older_tool_results();
        }
    ...
    fn prune_stale_skill_context(&mut self, current_turn_id: &str) {
        Arc::make_mut(&mut self.items).retain(|envelope| {
            if is_skill_catalog_context_item(&envelope.item) {
                // Full thread-start catalogs are exactly what adaptive mode
                // replaces, so an unscoped legacy catalog is safe to hide from
                // the model-visible snapshot.
                return envelope
                    .turn_id()
                    .is_some_and(|turn_id| turn_id == current_turn_id);
            }
            if is_selected_skill_context_item(&envelope.item) {
                return match envelope.turn_id() {
                    Some(turn_id) => turn_id == current_turn_id,
                    // Be conservative with unscoped selected skill prompts.
                    None => true,
                };
            }
            true
    ...
    fn prune_older_tool_results(&mut self) {
        let keep_recent = env_usize("CODEX_HYGIENE_KEEP_RECENT_TOOL_RESULTS", 8, 0, 256);
        let old_result_budget = env_usize("CODEX_HYGIENE_OLD_TOOL_RESULT_TOKENS", 1536, 128, 32768);
        let mut seen_tool_results = 0usize;
        for envelope in Arc::make_mut(&mut self.items).iter_mut().rev() {
            if let ResponseItem::FunctionCallOutput { output, .. }
            | ResponseItem::CustomToolCallOutput { output, .. } = &mut envelope.item
            {
                seen_tool_results = seen_tool_results.saturating_add(1);
                if seen_tool_results > keep_recent {
                    truncate_function_output_payload(
                        output,
                        TruncationPolicy::Tokens(old_result_budget),
