...
const DEFAULT_ADAPTIVE_SKILL_LIMIT: usize = 12;
...
fn adaptive_skill_catalog_enabled() -> bool {
    env_flag("CODEX_ADAPTIVE_SKILL_CATALOG")
}

fn adaptive_skill_catalog_limit() -> usize {
    std::env::var("CODEX_ADAPTIVE_SKILL_LIMIT")
        .ok()
        .and_then(|value| value.parse::<usize>().ok())
        .unwrap_or(DEFAULT_ADAPTIVE_SKILL_LIMIT)
        .clamp(1, 50)
}

fn adaptive_skill_always_include() -> HashSet<String> {
    std::env::var("CODEX_ADAPTIVE_SKILL_ALWAYS")
        .unwrap_or_default()
        .split(',')
        .map(str::trim)
...
fn normalized_skill_resource(resource: &str) -> String {
    resource.replace('\\', "/")
}
    ...
    C: Send + Sync + 'static,
{
    fn contribute_thread_context<'a>(
        ...
        Box::pin(async move {
            ...
            if !config.include_instructions || adaptive_skill_catalog_enabled() {
                return Vec::new();
            }
            let catalog = self
                .list_skills(
    ...
    C: Send + Sync + 'static,
{
    fn contribute<'a>(
        ...
        Box::pin(async move {
            ...
            let shadow_selection_turn = if config.shadow_selection_enabled || adaptive_enabled {
                let mut shadow_catalog = if adaptive_enabled {
                    selection_catalog.clone()
                } else {
                    catalog.clone()
                };
                if !adaptive_enabled {
            ...
            let adaptive_skill_resources = shadow_selection_turn
                .as_ref()
                .map(|state| state.current_turn_skill_resources(adaptive_skill_catalog_limit()))
                .unwrap_or_default();
            thread_state
            ...
            if (config.include_instructions || adaptive_enabled)
                && (!host_catalog_in_world_state || adaptive_enabled)
            {
                let mut turn_catalog = if adaptive_enabled {
                    selection_catalog.clone()
                } else {
                    catalog.clone()
                };
                let explicit_skill_names = selected_entries
                    .iter()
                    .map(|entry| entry.name.clone())
                    .collect::<HashSet<_>>();
                let always_include = adaptive_skill_always_include();
                turn_catalog.entries.retain(|entry| {
                    let normal_turn_entry = entry.authority.kind != SkillSourceKind::Executor
                        && entry.authority.kind != SkillSourceKind::Orchestrator;
                    if !normal_turn_entry {
                        return false;
                    }
                    if !adaptive_enabled {
                        return true;
                    }
                    explicit_skill_names.contains(&entry.name)
                        || always_include.contains(&entry.name)
                        || adaptive_skill_resources
                            .contains(&normalized_skill_resource(entry.main_prompt.as_str()))
                });
