...
impl ShadowSelectionTurnState {
    /// Return only the current-query lexical selection for active prompt projection.
    ///
    /// Deliberately avoid the LRU/task-history selectors here. Those remain useful
    /// for shadow evaluation, but using them for active projection would make old
    /// skills sticky after the task has moved on.
    pub(crate) fn current_turn_skill_resources(&self, limit: usize) -> HashSet<String> {
