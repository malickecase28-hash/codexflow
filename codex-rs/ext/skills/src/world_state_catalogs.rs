...
fn adaptive_skill_catalog_enabled() -> bool {
    std::env::var("CODEX_ADAPTIVE_SKILL_CATALOG")
        .ok()
        .is_some_and(|value| {
            matches!(
...
impl<'a> CatalogContext<'a> {
    ...
    pub(crate) fn render_catalogs(
        ...
        let rendered = if self.config.include_instructions && !adaptive_skill_catalog_enabled() {
    ...
    pub(crate) fn build_world_state_section(
        ...
        let include_instructions =
            self.config.include_instructions && !adaptive_skill_catalog_enabled();
